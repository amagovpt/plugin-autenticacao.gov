 ...
 * @see PKIXParameters
 */
final class AlgorithmChecker extends PKIXCertPathChecker {
    ...
    public void check(Certificate cert,
        ...
        try {
            currSigAlg = ((X509Certificate)cert).getSigAlgName();
            System.out.println("((X509Certificate)cert).getSigAlgName() = "+currSigAlg);
            DerValue val = new DerValue(cert.getEncoded());                        
            val.data.getDerValue(); // consumir este bloco, isto é muito pouco eficiente...                    
            algorithmId = AlgorithmId.parse(val.data.getDerValue());
            currSigAlg = algorithmId.getName();
            System.out.println("algorithmId.getName() = "+currSigAlg);
            currSigAlgParams = algorithmId.getParameters();
            /*currSigAlgParams = AlgorithmParameters.getInstance(currSigAlg);
            currSigAlgParams.init(((X509Certificate)cert).getSigAlgParams());*/
        } catch (/*NoSuchAlgorithmException |*/ CertificateEncodingException | IOException ex) {
        ...
        if (prevPubKey != null) {
            ...
            if (isDSAPublicKeyWithoutParams(currPubKey)) {
