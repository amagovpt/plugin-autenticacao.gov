...
class PlatformPCSC {
    private final static Logger LOGGER = Logger.getLogger(PlatformPCSC.class.getName());
    ...
    private static boolean loadPCSCLibrary (String sName)    
    {
        ...
        try {
            ...
            else if(isPlatformLinux())
            {
                ...
                throw new Exception("platform not detected");
            ...
            path = _theDll.getAbsolutePath();
            System.load(_theDll.getAbsolutePath());
            
            _theDll.deleteOnExit();
        } catch (Exception _e) 
        {
            LOGGER.log(Level.INFO, "Exceção: PlatformPCSC.loadPCSCLibrary path = {0}; nome = {1}; message = {2}; stacktrace = {3}", new Object[]{path, _sFinalName, _e.getMessage(), Utilities.stackTraceToString(_e)});
    ...
    private static File createTemporaryFile(String sName, String sExtension)
    {
        ...
        try {
            ...
            return _theDll;
             ...
             //System.out.println("createTemporaryFile:"+ex.getMessage());
             //ex.printStackTrace();          
            LOGGER.log(Level.INFO, "Exceção: PlatformPCSC.createTemporaryFile sname = {0}; sExtension = {1}; path = {2}; message = {3}; stacktrace = {4}", new Object[]{sName, sExtension, path, ex.getMessage(), Utilities.stackTraceToString(ex)});
            return null;
        }
    }
    ...
    private static Throwable loadLibrary() {
        try {
            ...
            LOGGER.log(Level.INFO, "Exceção: PlatformPCSC.loadLibrary message = {0}; stacktrace = {1}", new Object[]{e.getMessage(), Utilities.stackTraceToString(e)});
            // The library is not available
